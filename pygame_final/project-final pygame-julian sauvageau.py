...
def game():
    ...
    bgmusic = 'assets/Ade - Cryptik Dance.mp3'

    pygame.mixer.music.load(bgmusic)
    pygame.mixer.music.play(loops=-1, start=8)

    # Create a Sprite Group
    all_sprites_group = pygame.sprite.Group()
    block_sprites_group = pygame.sprite.Group()
    dmg_blocks_sprites_group = pygame.sprite.Group()
    ...
    # Create a damage block
    for _ in range(num_red_blocks):
        redblock = RedBlock()
        # choose a random position for it
        redblock.rect.centerx = random.randrange(0, WIDTH)
        redblock.rect.centery = random.randrange(0, HEIGHT)

        all_sprites_group.add(redblock)
        dmg_blocks_sprites_group.add(redblock)
    ...
    while not done:
        ...
        if not block_sprites_group:
            ...
            redblock.rect.centery = random.randrange(0, HEIGHT)
            all_sprites_group.add(redblock)
            block_sprites_group.add(redblock)
        ...
        # collision between player and red blocks
        dmg_block_collided = pygame.sprite.spritecollide(player, dmg_blocks_sprites_group, False)
        for redblock in dmg_block_collided:
            # decrease mario's life
            player.calc_damage(redblock.damage)
